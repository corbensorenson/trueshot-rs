...
- Status: In Progress (2026-02-08) — added real HDR bracket, focus stack, and HDR+focus stack capture endpoints and wired Camera Control Pro to trigger hardware sequences (still needs end‑to‑end hardware validation).
...
- Status: In Progress (P0 image-quality moat, researched 2026-07-26)
  ...
  - Added a deterministic adaptive acquisition core that evaluates camera-supported shutter/exact-ISO/focus candidates using calibrated radiance entropy reduction and SNR-coupled diopter uncertainty per millisecond; rejects motion, time, thermal, and missing-calibration violations before ranking; accounts for lens travel; and stops HDR and focus independently. Hardware-loop/manifest integration and the real 20% efficiency gate remain.
  ...
  - Remaining: calibration capture/fit workflow, empirical uncertainty coverage, held-out physical-depth/pupil/PSF and lens-breathing calibration, real halo-energy/MTF/mixed-pixel qualification, real dynamic-motion/disocclusion qualification, interactive source-map retouching, glare separation, and adaptive-planner hardware/manifest integration plus real efficiency qualification.
