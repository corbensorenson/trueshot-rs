...
- Status: In Progress (P0 image-quality moat, researched 2026-07-26)
  ...
  - Closed the retained Apple M1/Z9 full-sensor performance criterion. The gate now requires a clean tracked revision, exact requested geometry and decoded extent, admitted-memory compliance, oversized-arena release, source page-in amplification, and at least three independent production runs. At 21x8280x5520, wall p50/p95 is 88.556/91.206 seconds, stage p95 is 2.422 seconds decode, 41.703 seconds fusion, and 1.012 seconds Metal demosaic/postprocess; maximum RSS/footprint/energy is 3.637 GB/3.403 GB/609.782 J with nominal thermals. Exact artifacts and semantic provenance match across all runs. This does not close calibrated sensor/lens quality, redistributable-corpus, competitor, or cross-generation criteria.
  ...
  - Added exact executable replay capsules to new base/revision reports. Replay binds every quality, concurrency, ROI/full-frame, GPU, preview, deghost, frequency, glare, refusion, and optional calibration-profile choice to a strict schema; profiles must remain project-local and SHA-256 identical. The bounded stdin envelope carries the exact immutable base-report bytes plus the validated edit, so encrypted report/edit reads need no plaintext staging and stale or modified inputs fail closed.
  - Added the Advanced Capture-gated one-click local execution path. The server preflights real project directories, rejects symlink/path escape and unauthenticated/legacy encrypted RAW, reconstructs arguments only from the replay capsule, launches only the packaged sibling `trueshot` binary without a shell, bounds retained diagnostics, supports cancellation with child kill/reap, persists a non-retryable queue identity, audits execution/cancellation, and immediately encrypts revision artifacts when the output scope is protected.
  - The Inspector now keeps the feature visible behind its existing upgrade/trial pitch, queues a measured revision locally, polls persisted progress, supports cancellation, surfaces bounded failures, and refreshes the immutable revision family on success. Old reports without replay metadata remain viewable but deliberately require one base rerun before execution.
  - Exercised the production bounded replay envelope on the private 21-frame Z9 group. The run preserved the exact base report digest, emitted a distinct measured-only revision with one exact operator pixel, retained all replay controls, and produced digest-bound TIFF/report/map artifacts. Strict Rust Clippy for core/CLI/server, replay/edit/CLI/server focused tests, dashboard lint, and the production dashboard build pass. This integration record is `docs/benchmarks/local_fusion_revision_execution_qualification_2026-07-27.json`.
  - Added the Advanced Capture-gated measured acquisition console and a server-owned adaptive camera transaction. Session start now accepts only project-relative non-symlink reference/profile assets, loads a digest-bound exact-ISO profile locally, and binds camera/project/aperture identity into backward-compatible durable generations. The planner's numeric shutter/ISO decision resolves back to exact camera-declared strings, gPhoto applies exact readback and publishes the NEF directly into the authorized project RAW directory with fsync plus atomic rename, and the server measures elapsed time, selectively decodes and verifies the RAW, applies immediate project-policy encryption, then checkpoints the posterior. Failed validation or persistence cannot advance live state.
  - Adaptive dashboard recovery combines a local session pointer with the checksum-sealed server generation, exposes target focus in diopters/meters, shutter, ISO, HDR/focus information gain, predicted cost, worst posterior uncertainty, resource state, and attributable candidate exclusions, and requires explicit physical-focus confirmation before capture. Completed NEF metadata remains authoritative. An atomic RAII lease moves into the blocking hardware worker, keeping capture exclusive through disconnect/error/unwind until the physical action exits; legacy sessions remain inspectable but cannot automate capture. Project RAW inventory is opt-in, traversal-bounded before metadata/sorting, and deliberately excluded from legacy `all` listing to avoid million-file view regressions.
