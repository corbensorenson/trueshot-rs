...
use trueshot_core::tracking::{ObjectSegmenter, SegmentationConfig, MotionAnalyzer, MotionConfig, BoundingBox3D};
use nalgebra as na;
use uuid::Uuid;
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                for z in 0..size {
                    for y in 0..size {
                        for x in 0..size {
                            let pos = grid.index_to_world(x, y, z);
                            let dist = na::distance(&pos, &center) - radius;
                            grid.set(x, y, z, DensityVoxel { density: -dist });
                        }
                    }
                }
...
fn bench_voxel_grid(c: &mut Criterion) {
    ...
    group.bench_function("create_64x64x64", |b| {
        ...
        b.iter(|| {
            black_box(VoxelGrid::<f32>::new(min, max, [64, 64, 64]))
        });
    });
    ...
    group.bench_function("random_access_1000", |b| {
        let min = na::Point3::new(0.0, 0.0, 0.0);
        let max = na::Point3::new(1.0, 1.0, 1.0);
        let mut grid: VoxelGrid<f32> = VoxelGrid::new(min, max, [64, 64, 64]);
    ...
    for num_points in [100, 500, 1000].iter() {
        group.bench_with_input(
            ...
            |b, &num_points| {
                ...
                b.iter(|| {
                    black_box(segmenter.segment(&points))
                });
            },
        );
    }
...
fn bench_motion_analysis(c: &mut Criterion) {
    ...
    group.bench_function("predict_position", |b| {
        ...
        b.iter(|| {
            black_box(analyzer.predict(&id, 5.0))
        });
    });
...
fn bench_bounding_box(c: &mut Criterion) {
    ...
    group.bench_function("from_points_1000", |b| {
        ...
        b.iter(|| {
            black_box(BoundingBox3D::from_points(points.iter().cloned()))
        });
    });
