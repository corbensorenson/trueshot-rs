...
use trueshot_core::reconstruction::{
    MultiCamSfm, MultiCamConfig, CameraId, LivescanFrame, HighResImage,
};
use trueshot_sfm::{CameraIntrinsics, CameraPose, FeatureType, DistortionModel};
...
fn test_sd_card_ingestion() {
    ...
    let images: Vec<HighResImage> = (0..10).map(|i| {
        HighResImage {
