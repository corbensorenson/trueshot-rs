//! Integration tests for Hierarchical Collapse Pipeline
//!
//! Tests the full A/B/C grading and collapse pipeline.

use ndarray::{Array2, Array3};
use trueshot_core::hierarchical_grading::{GradingParams, grade_pixels, compute_grade_stats, Grade};
use trueshot_core::hierarchical_collapse::{
    HierarchicalParams, CollapseResult, SRFactor,
    collapse_c_grade, collapse_b_grade,
};
...
fn test_grading_produces_all_grades() {
    ...
    assert!(stats.count_a > 0, "Should have A-grade pixels (highest sharpness)");
    assert!(stats.count_c > 0, "Should have C-grade pixels (low sharpness foreground)");
...
fn test_c_grade_collapse_produces_valid_output() {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            assert!(val <= 2.0, "Value {} at ({}, {}) should be <= 2.0", val, y, x);
        }
    }
}
...
fn test_full_hierarchical_pipeline() {
    ...
    let exposures: Vec<f64> = (0..num_frames).map(|i| 1.0 + (i % 3) as f64 * 0.5).collect();
