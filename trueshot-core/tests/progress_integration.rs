...
use trueshot_core::progress::{
    CancellationToken, OperationEstimator, OperationPreview, ProcessingPhase, ProgressTracker,
};
...
fn test_cancellation_token_check() {
    ...
    assert!(
        token.check().is_ok(),
        "check() should succeed when not cancelled"
    );
...
fn test_cancellation_token_thread_safety() {
    ...
    assert!(
        token.is_cancelled(),
        "Cancellation should propagate across threads"
    );
}
...
fn test_cancellation_child_token() {
    ...
    assert!(
        parent.is_cancelled(),
        "Parent should see child cancellation"
    );
    assert!(child.is_cancelled());
}
...
fn test_operation_estimator_gaussian_splatting() {
    ...
    assert!(
        preview.estimated_seconds > preview.estimated_seconds * 0.0,
        "Positive time"
    );
}
...
fn test_operation_preview_formatting() {
    ...
    assert!(
        summary.contains("Test Operation"),
        "Summary should include name"
    );
    assert!(
        summary.contains("42 items"),
        "Summary should include item count"
    );
}
...
fn test_progress_tracker_phases() {
    ...
    // Start a phase
    tracker.start_phase(
        ProcessingPhase::ImageAnalysis,
        100,
        "Analyzing images".to_string(),
    );
