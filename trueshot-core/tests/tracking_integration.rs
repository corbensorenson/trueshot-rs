...
use trueshot_core::tracking::{
    ObjectSegmenter, SegmentationConfig, 
    MotionAnalyzer, MotionConfig, MotionClass,
    ObjectTracker, TrackerConfig, BoundingBox3D, SegmentedObject
};
use nalgebra as na;
use uuid::Uuid;
...
fn test_segmentation_finds_clusters() {
    ...
    for segment in &segments {
        assert!(!segment.point_indices.is_empty(), "Segment should have points");
    }
}
...
fn test_object_tracker_persistence() {
    ...
    let dets_frame1 = vec![make_segment(id1, det1.clone()), make_segment(id2, det2.clone())];
    ...
    let dets_frame3 = vec![make_segment(id1, det1_moved2), make_segment(id2, det2_moved2)];
