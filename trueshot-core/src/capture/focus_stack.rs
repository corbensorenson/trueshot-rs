...
use crate::Result;
use image::{DynamicImage, ImageBuffer, Rgb, GrayImage};
use rayon::prelude::*;
use std::path::Path;
...
impl FocusStacker {
    ...
    fn weighted_focus_stack(&self, images: &[DynamicImage]) -> Result<DynamicImage> {
        ...
        let gray_images: Vec<GrayImage> = images
            .iter()
    ...
    fn smooth_focus_map(&self, indices: &[usize], width: usize, height: usize, num_images: usize) -> Vec<Vec<f32>> {
        let kernel_size = 5;
                    ...
                    for img_idx in 0..num_images {
                        ...
                        for ky in 0..kernel_size {
                            for kx in 0..kernel_size {
                                ...
                                let dist = ((ky as f32 - half as f32).powi(2) + (kx as f32 - half as f32).powi(2)).sqrt();
    ...
    fn build_laplacian_pyramid(&self, img: &DynamicImage, levels: usize) -> Vec<Vec<[f32; 3]>> {
        ...
        // Convert to f32
        let mut current: Vec<[f32; 3]> = rgb.pixels()
        ...
        for level in 0..levels {
            ...
            let upsampled = self.upsample(&gaussian_pyramid[level + 1], next_width, next_height, level_width, level_height);
    ...
    fn upsample(&self, img: &[[f32; 3]], src_width: usize, src_height: usize, dst_width: usize, dst_height: usize) -> Vec<[f32; 3]> {
        ...
        for level in 0..=levels {
            ...
            for i in 0..pixel_count {
                ...
                for (img_idx, focus_pyramid) in focus_pyramids.iter().enumerate() {
                    if level < focus_pyramid.len() && i < focus_pyramid[level].len()
    ...
    fn reconstruct_from_pyramid(&self, pyramid: &[Vec<[f32; 3]>], target_width: usize, target_height: usize) -> Result<DynamicImage> {
        if pyramid.is_empty() {
        ...
        for level in (0..levels - 1).rev() {
            ...
            let upsampled = self.upsample(&current, prev_width.max(1), prev_height.max(1), level_width, level_height);
