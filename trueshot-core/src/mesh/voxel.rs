...
impl<T: VoxelData> VoxelGrid<T> {
    ...
    pub fn world_to_voxel(&self, pos: &na::Point3<f32>) -> Option<[usize; 3]> {
        ...
        if vx >= 0 && vx < self.dims[0] as isize
            && vy >= 0 && vy < self.dims[1] as isize
            && vz >= 0 && vz < self.dims[2] as isize
        {
    ...
    pub fn bounds(&self) -> (na::Point3<f32>, na::Point3<f32>) {
        let max = self.origin + na::Vector3::new(
...
impl VoxelData for TsdfVoxel {
    fn accumulate(&mut self, other: &Self) {
        if other.weight > 0.0 {
            ...
            for i in 0..3 {
                self.color[i] = (self.color[i] * self.weight + other.color[i] * other.weight) / w_sum;
            }
            self.weight = w_sum;
        }
    }
...
impl VoxelData for ConfidenceVoxel {
    ...
    fn lerp(&self, other: &Self, t: f32) -> Self {
        Self {
            view_count: ((self.view_count as f32 + (other.view_count as f32 - self.view_count as f32) * t) as u32),
...
mod tests {
    ...
    fn test_voxel_grid_creation() {
        let grid: DensityGrid = VoxelGrid::new(
            na::Point3::origin(),
    ...
    #[test]
    fn test_accumulate() {
        let mut grid: DensityGrid = VoxelGrid::new(
            na::Point3::origin(),
            na::Point3::new(1.0, 1.0, 1.0),
            0.1,
        );
        ...
        let voxel = grid.world_to_voxel(&na::Point3::new(0.5, 0.5, 0.5)).unwrap();
        assert_eq!(*grid.get(voxel), 1.5);
    }
    
    #[test]
    fn test_tsdf_accumulate() {
        let mut v1 = TsdfVoxel { tsdf: 0.5, weight: 1.0, color: [1.0, 0.0, 0.0] };
        let v2 = TsdfVoxel { tsdf: -0.5, weight: 1.0, color: [0.0, 1.0, 0.0] };
