...
pub fn export_texture_compressed(img: &DynamicImage, path: &Path, format: &str) -> Result<()> {
    ...
    match format {
        ...
        "bc3" => {
            let compressed_data = intel_tex_2::bc3::compress_blocks(&surface);
            write_dds_file(path, width, height, &compressed_data, "DXT5", 77)?;
        }
        "bc1" => {
            let compressed_data = intel_tex_2::bc1::compress_blocks(&surface);
            write_dds_file(path, width, height, &compressed_data, "DXT1", 71)?;
        }
        _ => {
            // Default to PNG
            img.save(path)
                .context("Failed to save standard image format")?;
        }
    }
    Ok(())
}
