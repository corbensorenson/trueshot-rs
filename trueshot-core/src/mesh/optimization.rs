...
pub fn remove_statistical_outliers(
    ...
    tracing::info!("Removing statistical outliers from {} points...", points.len());
    ...
    tracing::info!("Removed {} outliers ({:.1}%), kept {} points", 
               removed, (removed as f32 / points.len() as f32) * 100.0, filtered.len());
...
pub fn remove_radius_outliers(
    ...
    let removed = points.len() - filtered.len();
    tracing::info!("Removed {} radius outliers ({:.1}%), kept {} points", 
               removed, (removed as f32 / points.len() as f32) * 100.0, filtered.len());
...
pub fn filter_by_confidence(points: &[ColoredPoint], min_confidence: f32) -> Vec<ColoredPoint> {
    ...
    let removed = points.len() - filtered.len();
    if removed > 0 {
        tracing::info!("Removed {} low-confidence points ({:.1}%)", 
                   removed, (removed as f32 / points.len() as f32) * 100.0);
    }
...
pub fn voxel_downsample(points: &[ColoredPoint], voxel_size: f32) -> Vec<ColoredPoint> {
    ...
    tracing::info!("Downsampling {} points with voxel size {:.4}m...", points.len(), voxel_size);
    ...
    let downsampled: Vec<ColoredPoint> = voxel_map
        ...
        .map(|voxel_points| {
            ...
            let avg_pos = voxel_points
                .iter()
                .fold(na::Vector3::zeros(), |acc, p| acc + p.position.coords) / n;
    ...
    tracing::info!("Downsampled to {} points ({:.1}% reduction)", 
               downsampled.len(), 
...
pub fn remove_small_components(mesh: &mut Mesh, min_faces: usize) {
    ...
    tracing::info!("Removing small mesh components (min {} faces)...", min_faces);
    ...
    for start_idx in 0..mesh.faces.len() {
        ...
        while let Some(face_idx) = queue.pop() {
            ...
            for other_idx in 0..mesh.faces.len() {
                ...
                let shared_vertices = face.vertices.iter()
