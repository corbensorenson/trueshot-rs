...
pub fn collapse_foreground_single_pass(
...
) -> Result<Array2<f64>> {  // Returns BAYER (H×W), not RGB
    if frames.is_empty() {
    ...
    let fg_pixels: Vec<(usize, usize)> = grades.indexed_iter()
        .filter(|(_, &g)| g == Grade::A as u8 || g == Grade::B as u8 || g == Grade::C as u8)
        .map(|((y, x), _)| (y, x))
        .collect();

    tracing::info!("Collapsing {} foreground pixels (A+B+C) in single pass", fg_pixels.len());
    ...
    if let Some(best_plane) = best_plane_map {
        ...
        let image_weights: Vec<f64> = frames.iter()
            .map(|frame| {
        ...
        tracing::info!("Using exposure-aware HDR weights for {} exposures: {:?}", num_images, image_weights);
        ...
        use rayon::prelude::*;
        let results: Vec<(usize, usize, f64)> = fg_pixels.par_iter()
            .map(|&(y, x)| {
        ...
        tracing::info!("Collapsing foreground pixels (baseline weighted average - no focus plane info)");
    ...
    tracing::info!("Single-pass foreground collapse complete: {} pixels (Bayer output)", fg_pixels.len());
...
pub fn collapse_c_grade(
    ...
    let c_pixels: Vec<(usize, usize)> = grades.indexed_iter()
    ...
    if let Some(best_plane) = best_plane_map {
        ...
        // Parallel processing: for each pixel, use only frames from best focus plane
        use rayon::prelude::*;
        let results: Vec<(usize, usize, f64)> = c_pixels.par_iter()
            .map(|&(y, x)| {
                let plane_idx = best_plane[[y, x]];
                let start_frame = plane_idx * num_exposures;
                let end_frame = start_frame + num_exposures;
        ...
        // Fallback: old behavior (average across all frames)
        tracing::info!("Collapsing C-grade pixels (baseline weighted average - no focus plane info)");
    ...
    tracing::info!("C-grade collapse complete: {} pixels (Bayer output)", c_count);
...
pub fn collapse_b_grade(
    ...
    if frames.is_empty() { anyhow::bail!("No frames"); }
    let (height, width, _) = frames[0].dim();
    let num_images = frames.len();
    let mut result = Array2::<f64>::zeros((height, width));

    let b_pixels: Vec<(usize, usize)> = grades.indexed_iter()
    ...
    if let Some(best_plane) = best_plane_map {
        use rayon::prelude::*;
        let results: Vec<(usize, usize, f64)> = b_pixels.par_iter()
            .map(|&(y, x)| {
                let plane_idx = best_plane[[y, x]];
                let start_frame = plane_idx * num_exposures;
                let end_frame = start_frame + num_exposures;
                ...
                for n in start_frame..end_frame.min(num_images) {
                    let pixel_value = frames[n][[y, x, 0]];
                    let sigma = 1.0; let target = 1.0;
                    let w = if pixel_value < 0.01 { 0.01 } else if pixel_value > 5.0 { 0.01 }
                    else { (-(pixel_value - target).powi(2) / (2.0 * sigma * sigma)).exp() };
                    pixel_sum += w * pixel_value;
                    weight_sum += w;
                }
                let y_b = if weight_sum > 1e-10 { pixel_sum / weight_sum } else { 0.0 };
                let x_c = c_result[[y, x]];
                let value = (y_b + lambda * x_c) / (1.0 + lambda);
                (y, x, value)
            })
            .collect();
        for (y, x, value) in results { result[[y, x]] = value; }
    } else {
        // Fallback without planes (average all)
        use rayon::prelude::*;
        let results: Vec<(usize, usize, f64)> = b_pixels.par_iter()
            .map(|&(y, x)| {
                let mut pixel_sum = 0.0; let mut weight_sum = 0.0;
                for n in 0..num_images {
                    let pixel_value = frames[n][[y, x, 0]];
                    pixel_sum += pixel_value; weight_sum += 1.0;
                }
                let y_b = if weight_sum > 1e-10 { pixel_sum / weight_sum } else { 0.0 };
                let x_c = c_result[[y, x]];
                let value = (y_b + lambda * x_c) / (1.0 + lambda);
...
pub fn collapse_bc_grade_frames(
...
) -> Result<(Array3<f64>, Array3<f64>)> {  // Returns (B result, C result) both RGB (H×W×3)
    if frames.is_empty() {
        anyhow::bail!("No frames provided");
    }
    ...
    let bc_pixels: Vec<(usize, usize)> = grades.indexed_iter()
    ...
    if let Some(best_plane) = best_plane_map {
        tracing::info!("Collapsing B+C grades together (joint demosaicing, {} B pixels, {} C pixels)",
            b_pixels.len(), c_pixels.len());
        ...
        tracing::info!("B+C collapse complete: {} B pixels, {} C pixels (RGB output)",
            b_pixels.len(), c_pixels.len());
...
pub fn collapse_a_grade(
    ...
    let has_subpixel_diversity = if let Some(aligns) = alignments {
        ...
        let max_shift = dx_values.iter().chain(dy_values.iter())
            .map(|v| v.abs())
            .fold(0.0f64, f64::max);
        ...
        tracing::info!("Subpixel diversity check: max_shift={:.3}px, scale_range={:.4}",
                      max_shift, scale_range);
    ...
    if sr_factor > 1.0 {
        ...
        if !has_subpixel_diversity {
            tracing::warn!("No subpixel diversity detected - SR requires subpixel shifts between frames");
            tracing::warn!("Using native resolution instead (focus stacking + HDR only)");
        }
    }

    tracing::info!("Collapsing A+B+C grades for SR (A={:.1}%, B={:.1}%, C={:.1}%, total={:.1}%, SR={:.3}x{})",
                   ...
                   if do_sr { "" } else { " [DISABLED - sparse data]" });
    ...
    if do_sr {
        ...
        let rgb = collapse_a_grade_with_sr(&stack, grades, b_result, c_result, exposures, params, alignments.unwrap())?;
        ...
        let bayer = collapse_a_grade_native_frames(frames, grades, b_result, c_result, exposures, params, best_plane_map, num_exposures, wb_multipliers, alignments)?;
...
fn collapse_a_grade_native_frames(
    ...
    let a_pixels: Vec<(usize, usize)> = grades.indexed_iter()
    ...
    // Check if we have focus plane information
    if let Some(best_plane) = best_plane_map {
        tracing::info!("A-grade collapse: per-pixel focus plane selection + HDR fusion (guided by B prior)");
        ...
        // Parallel processing: for each pixel, use only frames from best focus plane
        use rayon::prelude::*;
        let results: Vec<(usize, usize, f64)> = a_pixels.par_iter()
            .map(|&(y, x)| {
                let plane_idx = best_plane[[y, x]];
                let start_frame = plane_idx * num_exposures;
                let end_frame = start_frame + num_exposures;
        ...
        // Fallback: old behavior (average across all frames)
        tracing::info!("A-grade collapse: baseline weighted average (guided by B prior - no focus plane info)");
    ...
    tracing::info!("A-grade collapse complete: {} pixels (Bayer output, blended with B prior)", a_count);
...
fn joint_sr_demosaic(
    ...
    tracing::info!("Joint SR+demosaic: {}x{} → {}x{} RGB ({}x)",
                   width, height, sr_w, sr_h, sr_factor);
    ...
    tracing::info!("Grade counts in joint_sr_demosaic: A={}, B={}, C={}, D={}",
    ...
    tracing::info!("Unique alignments: {} out of {} frames", unique_alignments.len(), num_images);
    ...
    for n in 0..num_images {
        ...
        if n < 5 {
            tracing::info!("Frame {} alignment: dx={:.3}, dy={:.3} (SR: dx_sr={:.3}, dy_sr={:.3})",
                          n, align.dx, align.dy, dx_sr, dy_sr);
        }
    ...
    tracing::info!("Pixel stats: passed_grade={}, accumulated={}, out_of_bounds={}",
                   pixels_passed_grade, pixels_accumulated, pixels_out_of_bounds);
    ...
    tracing::info!("Weight stats: total_weight={:.3}, max_weight={:.6}", total_weight, max_weight);
    ...
    let r_filled = rgb_weights.slice(s![0, .., ..]).iter().filter(|&&w| w > 1e-10).count();
    let g_filled = rgb_weights.slice(s![1, .., ..]).iter().filter(|&&w| w > 1e-10).count();
    let b_filled = rgb_weights.slice(s![2, .., ..]).iter().filter(|&&w| w > 1e-10).count();
    let total_pixels = sr_h * sr_w;

    tracing::info!("RGB channel coverage: R={:.1}%, G={:.1}%, B={:.1}%",
    ...
    tracing::warn!("Coverage is too low! With {} frames and subpixel shifts, expected ~{}% per channel",
                   num_images, (r_filled as f64 / total_pixels as f64) * 100.0 * num_images as f64);
...
fn interpolate_missing_rgb_channels(
    rgb: &mut Array3<f64>,
    weights: &Array3<f64>,
) -> Result<()> {
    ...
    for pass in 0..max_passes {
                ...
                for c in 0..3 {
                    if weights_read[[c, y, x]] < 1e-10 {
                        ...
                        if !neighbors.is_empty() {
                            ...
                            let mut values: Vec<f64> = neighbors.iter().map(|(v, _, _)| *v).collect();
        ...
        if pass < 5 || pass % 10 == 0 {
            tracing::debug!("RGB interpolation pass {}: filled {} pixels", pass, filled_this_pass);
        }
    }
...
fn fill_upsampled_holes(array: &mut Array2<f64>) -> Result<()> {
    ...
    let mut non_zero_values: Vec<f64> = array.iter()
        .filter(|&&v| v > 1e-10)
        .copied()
        .collect();
    ...
    if filled > 0 {
        tracing::debug!("Filled {} holes in upsampled array with median={:.4}", filled, median);
    }
...
pub fn merge_graded_results(
    ...
    match a_result {
        ...
        CollapseResult::Rgb(_rgb_a) => {
            ...
            anyhow::bail!("Unexpected RGB result from A-grade collapse - all grades should return Bayer now");
        }
    }
}
...
fn merge_with_sr(
    ...
    tracing::info!("Bayer-aware merge: upsampling B/C from {}x{} to {}x{}",
    ...
    if a_total_count > 0 {
        ...
        tracing::info!("A-grade SR has {} holes / {} total ({:.1}%) - will fill in RGB after demosaic",
                      a_black_count, a_total_count, black_pct);
    }

    tracing::info!("Bayer-aware merge complete: output {}x{}", hr_width, hr_height);
...
pub fn compute_mertens_weights_frames(
    ...
    use rayon::prelude::*;
    let results: Vec<(f64, f64, f64)> = frames.par_iter()
        .map(|frame| {
            ...
            for y in (step..height-step).step_by(step) {
                for x in (step..width-step).step_by(step) {
                    let lap = (4.0 * frame[[y, x, 0]]
                        ...
                        - frame[[y+step, x, 0]]
                        - frame[[y, x-step, 0]]
                        - frame[[y, x+step, 0]]).abs();
                    lap_sum += lap;
                    count += 1;
                }
            }
            let contrast = if count > 0 { lap_sum / count as f64 } else { 0.0 };
            ...
            let saturation = if frame.dim().2 >= 3 {
                ...
                if sat_count > 0 { sat_sum / sat_count as f64 } else { 1.0 }
            } else {
                1.0
            };
...
fn stack_frames_scalar(frames: &[Array3<f64>]) -> Result<Array3<f64>> {
    ...
    for (idx, frame) in frames.iter().enumerate() {
        ...
        stacked.slice_mut(s![.., .., idx]).assign(&frame.slice(s![.., .., 0]));
    }
    Ok(stacked)
}
