...
use crate::scanning::workflow::{ScanWorkflow, ScanAction}; // Import Workflow types
...
// New Task System
use crate::scanning::tasks::{ScanTask, DirectorContext};
use crate::scanning::tasks::hardware::{HomeTurntableTask, VerifyHardwareTask, CheckExposureTask, CheckCenteringTask};
use crate::scanning::tasks::workflow::{PromptUserTask, CalibrateTask, SmartScanTask, StartProcessingTask, BackgroundScanTask};
...
impl Director {
    pub fn new(bus: Arc<EventBus>, cameras: Arc<Mutex<CameraManager>>, turntable: Box<dyn Turntable>, scheduler: Option<Arc<Scheduler>>) -> Self {
        Self {
    ...
    fn execute_step<'a>(&'a self, step_idx: usize) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send + 'a>> {
        Box::pin(async move {
            ...
            if let Some(wf) = wf_guard.as_ref() {
                if step_idx >= wf.steps.len() {
                    ...
                    *state = DirectorState::Idle;
                    self.bus.publish(SystemEvent::SystemMessage("Workflow Complete".into(), LogLevel::Success));
                ...
                let task_complete = {
                     ...
                     if let Some(t) = task_guard.as_ref() {
                         match t.on_enter(&ctx).await {
                             ...
                             Err(e) => {
                                 // Handle Error
                                 self.bus.publish(SystemEvent::SystemMessage(format!("Step Error: {}", e), LogLevel::Error));
                                 let mut state = self.state.lock().await;
    ...
    pub async fn stop_scan(&self) -> Result<()> {
        ...
        *task = None;
        ...
        self.bus.publish(SystemEvent::SystemMessage("Scan Aborted by User".into(), LogLevel::Warning));
        Ok(())
    }
        ...
        if let Some(task) = task_guard.as_ref() {
             ...
             match task.on_frame(&ctx, frame).await {
                 ...
                 Err(e) => {
                     ...
                     let mut state = self.state.lock().await;
                     *state = DirectorState::Error(e.to_string());
                     self.bus.publish(SystemEvent::SystemMessage(format!("Task Error: {}", e), LogLevel::Error));
                 }
             }
        }
    }
