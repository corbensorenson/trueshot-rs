...
impl S3Client {
    pub fn new(bucket_name: &str, region_name: &str, endpoint: Option<&str>) -> Result<Self> {
        let region = if let Some(ep) = endpoint {
            Region::Custom { region: region_name.to_string(), endpoint: ep.to_string() }
        } else {
            match region_name {
                "us-east-1" => Region::UsEast1,
                _ => Region::Custom { region: region_name.to_string(), endpoint: "https://s3.amazonaws.com".to_string() }
            }
        };
...
impl CloudStorage for S3Client {
    fn upload_file(&self, local_path: &Path, remote_path: &str) -> Result<()> {
        tracing::info!("Uploading {:?} to s3://{}/{}", local_path, self.bucket.name, remote_path);
    ...
    fn upload_directory(&self, local_dir: &Path, remote_prefix: &str) -> Result<()> {
        tracing::info!("Syncing {:?} to s3://{}/{}", local_dir, self.bucket.name, remote_prefix);
    ...
    fn download_file(&self, remote_path: &str, local_path: &Path) -> Result<()> {
        tracing::info!("Downloading s3://{}/{} to {:?}", self.bucket.name, remote_path, local_path);

        let rt = tokio::runtime::Runtime::new()?;
        let data = rt.block_on(self.bucket.get_object(remote_path))
