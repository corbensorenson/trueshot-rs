...
impl ClusterManager {
    ...
    pub async fn add_node(&self, url: String) -> Result<Uuid> {
        // Ping node
        let resp = self.client.get(format!("{}/health", url))
            .send()
            .await
    ...
    pub async fn dispatch_job(&self, job: Box<dyn Job + Send + Sync>) -> Result<Uuid> {
        ...
        for node in nodes.values_mut() {
            if node.status == NodeStatus::Idle {
                let payload = job.remote_payload()
                    .ok_or_else(|| anyhow::anyhow!("Job type cannot be serialized for remote dispatch"))?;
                ...
                if !resp.status().is_success() {
                    ...
                    return Err(anyhow::anyhow!("Remote job dispatch failed: {} {}", status, body));
                }
