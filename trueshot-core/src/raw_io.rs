...
pub fn load_bayer_frame(path: &Path) -> Result<BayerFrame> {
    ...
    tracing::debug!("NEF metadata: {}x{}, {} bits, compression={}",
                   metadata.width, metadata.height,
                   metadata.bits_per_sample, metadata.compression);

    // Load full raw data
    let raw_buffer = parser.load_full()
        .context("Failed to load full raw data")?;

    tracing::debug!("Loaded {} pixels in {:.1}ms",
                   raw_buffer.data.len(),
...
///
/// Uses Z9NefParser to decompress ONLY the specified rectangle, avoiding
/// full image decode. This provides 70-90% speedup for small ROIs.
pub fn selective_bayer_load(path: &Path, rect: &Rect) -> Result<BayerFrame> {
    ...
    tracing::debug!("ROI: {}x{} at ({}, {}) - {:.1}% of full image",
    ...
    let raw_buffer = parser.load_roi(&roi, None)
        .context("Failed to load ROI")?;

    tracing::info!("Loaded ROI: {}x{} ({} pixels) in {:.1}ms",
                  raw_buffer.width, raw_buffer.height, raw_buffer.data.len(),
                  start.elapsed().as_secs_f64() * 1000.0);
...
fn convert_raw_buffer_to_bayer_frame(
    ...
    let exposure_ev = (shutter_speed / (1.0 / 125.0)).log2()
    ...
    tracing::info!("Exposure: EV={:.2}, scale={:.3}x (shutter={:.6}s, ISO={}, f/{:.1})",
        exposure_ev, exposure_scale, shutter_speed, iso, aperture);
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let linear = ((pixel_value - BLACK_LEVEL) / (WHITE_LEVEL - BLACK_LEVEL))
                .max(0.0);
    ...
    for y in 0..height {
        for x in 0..width {
            let val = bayer[[y, x, 0]];
            match (y % 2, x % 2) {
                (0, 0) => { r_sum += val; r_count += 1; }  // R
                (0, 1) | (1, 0) => { g_sum += val; g_count += 1; }  // G
                (1, 1) => { b_sum += val; b_count += 1; }  // B
                _ => {}
            }
        }
    }
    ...
    tracing::debug!("Raw Bayer stats (file: {:?}): R={:.4}, G={:.4}, B={:.4}, R/G={:.3}, B/G={:.3}",
        path.file_name(), r_avg, g_avg, b_avg, r_avg/g_avg, b_avg/g_avg);
...
pub fn load_bayer_frame_with_detection(path: &Path) -> Result<BayerFrame> {
    ...
    // Try to detect object bbox
    match crate::object_detection::detect_object_bbox(path) {
        Ok(rect) => {
            tracing::info!("Object detected, using selective loading");
            selective_bayer_load(path, &rect)
        }
        Err(e) => {
