...
impl ScanTask for VerifyHardwareTask {
    ...
    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
        ...
        if connected == 0 {
            ctx.bus.publish(SystemEvent::SystemMessage(
                "No cameras detected! Check connections.".into(),
                LogLevel::Error,
            ));
            let mut state = ctx.state.lock().await;
            *state = DirectorState::Error("No cameras detected".into());
            ...
            ctx.bus.publish(SystemEvent::SystemMessage(
                format!("Verified {} cameras.", connected),
                LogLevel::Success,
            ));
            Ok(true)
        }
...
impl ScanTask for CheckExposureTask {
    ...
    async fn on_frame(
        ...
        if avg_lum < 20.0 {
            ctx.bus.publish(SystemEvent::SystemMessage(
                "Scene too dark! Increase lighting.".into(),
                LogLevel::Error,
            ));
            let mut state = ctx.state.lock().await;
            *state = DirectorState::Error("Exposure Check Failed: Too Dark".into());
            ...
            ctx.bus.publish(SystemEvent::SystemMessage(
                "Scene Overexposed! Reduce lighting.".into(),
                LogLevel::Error,
            ));
            let mut state = ctx.state.lock().await;
...
impl ScanTask for CheckCenteringTask {
    ...
    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
        ctx.bus.publish(SystemEvent::SystemMessage(
            "Checking Object Centering...".into(),
            LogLevel::Info,
        ));
        Ok(false)
    }
    ...
    async fn on_frame(
        ...
        if let Some(bg) = bg_guard.as_ref() {
            ...
            if mass < 100.0 {
                ...
                if dist > max_dist {
                    ctx.bus.publish(SystemEvent::SystemMessage(
                        "Object Off-Center! Please center it.".into(),
                        LogLevel::Error,
                    ));
                    let mut state = ctx.state.lock().await;
            ...
            ctx.bus.publish(SystemEvent::SystemMessage(
                "Skipping Centering Check (No BG)".into(),
                LogLevel::Warning,
            ));
            Ok(true)
        }
