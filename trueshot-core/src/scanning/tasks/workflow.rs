...
use crate::scanning::{SmartScanStrategy, QualityLevel};
use crate::scanning::workflow::{ScanAction, CaptureConfig};
use anyhow::Result;
...
impl ScanTask for PromptUserTask {
    fn name(&self) -> &'static str { "PromptUser" }

    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
        ctx.bus.publish(SystemEvent::SystemMessage(self.message.clone(), LogLevel::Warning));
    ...
    async fn on_frame(&self, ctx: &DirectorContext, frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<bool> {
        let mut detector = ctx.detector.lock().await;
        if detector.update(frame) {
            ctx.bus.publish(SystemEvent::SystemMessage("Scene Stabilized. Proceeding.".into(), LogLevel::Success));
...
impl ScanTask for CalibrateTask {
    fn name(&self) -> &'static str { "Calibrate" }

    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
        ctx.bus.publish(SystemEvent::SystemMessage("Auto-Calibrating...".into(), LogLevel::Info));
        ...
        ctx.bus.publish(SystemEvent::SystemMessage(
            format!("Optimized: {:.1} deg step, {} focus slices.", result.turntable_step_deg, result.focus_steps), 
            LogLevel::Success
        ));
        ...
        let bracket_str = if brackets.len() > 1 { format!(" | Auto-HDR: {:?}", brackets) } else { "".into() };
        ctx.bus.publish(SystemEvent::SystemMessage(
            format!("Est. Time: {:.1} min ({} shots){}", total_sec / 60.0, shots, bracket_str), 
            LogLevel::Info
        ));
        ...
        if let Some(wf) = wf_guard.as_mut() {
             for step in &mut wf.steps {
                 if let ScanAction::SmartScan { capture, quality: q } = step {
        ...
        ctx.bus.publish(SystemEvent::SystemMessage("Switched to SD Card Storage.".into(), LogLevel::Info));
...
impl ScanTask for StartProcessingTask {
    fn name(&self) -> &'static str { "StartProcessing" }

    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
        ctx.bus.publish(SystemEvent::SystemMessage("Starting Unified Reconstruction...".into(), LogLevel::Info));
        ...
        if let Some(scheduler) = &ctx.scheduler {
            let proj = ctx.project.lock().await;
             if let Some(p) = proj.as_ref() {
                 ...
                 ctx.bus.publish(SystemEvent::SystemMessage("Unified processing started.".into(), LogLevel::Info));
             }
        }
        Ok(true)
    }
...
impl ScanTask for SmartScanTask {
    fn name(&self) -> &'static str { "SmartScan" }

    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
         let mut strat = ctx.strategy.lock().await;
         *strat = SmartScanStrategy::new(self.quality);
         ctx.bus.publish(SystemEvent::SystemMessage(format!("Starting Smart Scan ({:?})", self.quality), LogLevel::Info));
         ctx.bus.publish(SystemEvent::CaptureStarted(self.step_idx as u32));
         Ok(false) // Wait for frames to drive the scan
    }

    async fn on_frame(&self, ctx: &DirectorContext, _frame: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<bool> {
        let mut strat = ctx.strategy.lock().await;
        ...
        if let Some(next_angle) = strat.next_angle() {
             // Rotate
             {
                 ...
                 if let Err(e) = tt.rotate_to(next_angle).await {
                      ctx.bus.publish(SystemEvent::SystemMessage(format!("Turntable error: {}", e), LogLevel::Error));
                      return Ok(false); // Check if we should error hard?
                 }
             }

             // Capture
             self.do_capture_sequence(ctx, &self.capture, next_angle).await?;
             
             strat.visit(next_angle);
             ctx.bus.publish(SystemEvent::CaptureProgress(self.step_idx as u32, next_angle / 360.0));
             Ok(false) // Continue
        } else {
...
impl ScanTask for BackgroundScanTask {
    fn name(&self) -> &'static str { "BackgroundScan" }
    
    async fn on_enter(&self, ctx: &DirectorContext) -> Result<bool> {
         let mut strat = ctx.strategy.lock().await;
         *strat = SmartScanStrategy::new(QualityLevel::Preview);
         ctx.bus.publish(SystemEvent::SystemMessage("Capturing Background...".into(), LogLevel::Info));
         Ok(false)
    }
...
impl SmartScanTask {
    async fn do_capture_sequence(&self, ctx: &DirectorContext, config: &CaptureConfig, angle: f32) -> Result<()> {
...
async fn perform_capture(ctx: &DirectorContext, config: &CaptureConfig, angle: f32) -> Result<()> {
    ...
    // Note: Simplified for brevity, assume similar structure to original code
     ...
     for cam in &active {
         ...
         match cam.capture(&capture_config) {
             ...
             Err(e) => {
                 ctx.bus.publish(SystemEvent::SystemMessage(format!("Capture failed for {}: {}", cam.id(), e), LogLevel::Warning));
             }
         }
     }
     drop(mgr);
    ...
    if let Some(s) = session.as_mut() {
        s.log_capture(angle, captured_ids.clone(), &format!("{:?}", config), captured_count);
...
async fn write_calibration_overrides(
    ...
    if wrote_any {
        let candidates = active_ids
            .iter()
            .filter_map(|id| mgr.registry.get_profile(id).and_then(|p| p.calibration.as_ref()))
            .collect::<Vec<_>>();
        if candidates.len() == 1 {
