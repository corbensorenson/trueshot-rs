...
impl FrameCache {
    ...
    pub fn get_interpolation_frames(&self, time: f32) -> Option<(&Vec<SlicedGaussian3D>, &Vec<SlicedGaussian3D>, f32)> {
...
impl GpuRasterizer4D {
    ...
    pub fn render_with_camera(
        ...
        let (color_buffer, depth_buffer) = self.tile_based_render(&projected, camera.width as usize, camera.height as usize);
    ...
    fn interpolate_frames(
        ...
        for p_gaussian in prev {
            ...
            for n_gaussian in next {
                ...
                if dist < best_dist && dist < 1.0 {  // Max 1 unit displacement
                    best_dist = dist;
        ...
        for n_gaussian in next {
            let has_match = prev.iter().any(|p| {
                na::distance(&p.position, &n_gaussian.position) < 1.0
            });
    ...
    fn project_gaussians(
        ...
        sliced.par_iter().map(|g| {
            ...
            let cov2d = self.project_covariance(&g.covariance, cam.z.max(0.001), fx.max(1.0), fy.max(1.0));
    ...
    fn project_covariance(&self, cov3d: &na::Matrix3<f32>, depth: f32, fx: f32, fy: f32) -> na::Vector3<f32> {
    ...
    fn tile_based_render(
        ...
        sorted.sort_by(|a, b| b.depth.partial_cmp(&a.depth).unwrap_or(std::cmp::Ordering::Equal));
        ...
        for (idx, g) in sorted.iter().enumerate() {
            for ty in g.tile_min[1]..=g.tile_max[1] {
                for tx in g.tile_min[0]..=g.tile_max[0] {
                    ...
                    if tile_idx < tiles.len() && tiles[tile_idx].len() < self.config.base_config.max_gaussians_per_tile as usize {
        ...
        for (tile_idx, tile_gaussians) in tiles.iter().enumerate() {
            if tile_gaussians.is_empty() { continue; }
            ...
            for py in 0..tile_size {
                for px in 0..tile_size {
                    ...
                    for &g_idx in tile_gaussians {
                        ...
                        if gaussian_alpha < 0.01 { continue; }
