...
impl HexPlaneEncoder {
    ...
    pub fn update(&mut self, gradients: &[f32], learning_rate: f32) {
        for (feat, &grad) in self.planes.iter_mut()
...
impl DeformationMLP {
    ...
    pub fn forward(&self, input: &[f32]) -> DeformationOutput {
        ...
        DeformationOutput {
            ...
            rotation_delta: na::UnitQuaternion::from_quaternion(
                na::Quaternion::new(output[6], output[3], output[4], output[5])
            ),
...
impl LifespanManager {
    ...
    pub fn add_gaussians(&mut self, count: usize) {
        ...
        for i in 0..count {
            self.lifespans.push(GaussianLifespan::new(
                start_id + i,
                self.current_frame,
            ));
        }
    ...
    pub fn statistics(&self) -> LifespanStatistics {
        ...
        let active = self.lifespans.iter()
            .filter(|l| self.current_frame - l.last_active_frame <= 5)
            .count();
        let avg_lifespan = self.lifespans.iter()
...
mod tests {
    ...
    fn test_lifespan_manager() {
        ...
        for _ in 0..50 {
            let opacities: Vec<f32> = (0..100)
                .map(|i| if i < 50 { 0.5 } else { 0.001 })
                .collect();
