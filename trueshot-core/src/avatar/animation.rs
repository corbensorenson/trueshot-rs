...
impl ARKitBlendshapes {
    ...
    pub fn to_array(&self) -> [f32; 52] {
        [
            ...
            self.eye_squint_left, self.eye_squint_right,
            self.eye_wide_left, self.eye_wide_right,
            self.brow_down_left, self.brow_down_right,
            self.brow_inner_up,
            self.brow_outer_up_left, self.brow_outer_up_right,
            self.nose_sneer_left, self.nose_sneer_right,
            self.cheek_puff,
            self.cheek_squint_left, self.cheek_squint_right,
            self.mouth_close, self.mouth_funnel, self.mouth_pucker,
            self.mouth_left, self.mouth_right,
            self.mouth_smile_left, self.mouth_smile_right,
            self.mouth_frown_left, self.mouth_frown_right,
            self.mouth_dimple_left, self.mouth_dimple_right,
            self.mouth_stretch_left, self.mouth_stretch_right,
            self.mouth_roll_lower, self.mouth_roll_upper,
            self.mouth_shrug_lower, self.mouth_shrug_upper,
            self.mouth_press_left, self.mouth_press_right,
            self.mouth_lower_down_left, self.mouth_lower_down_right,
            self.mouth_upper_up_left, self.mouth_upper_up_right,
            self.jaw_forward, self.jaw_left, self.jaw_right, self.jaw_open,
            self.tongue_out,
        ]
    }
    ...
    pub fn from_array(arr: &[f32; 52]) -> Self {
        Self {
            eye_blink_left: arr[0], eye_blink_right: arr[1],
            eye_look_down_left: arr[2], eye_look_down_right: arr[3],
            eye_look_in_left: arr[4], eye_look_in_right: arr[5],
            eye_look_out_left: arr[6], eye_look_out_right: arr[7],
            eye_look_up_left: arr[8], eye_look_up_right: arr[9],
            eye_squint_left: arr[10], eye_squint_right: arr[11],
            eye_wide_left: arr[12], eye_wide_right: arr[13],
            brow_down_left: arr[14], brow_down_right: arr[15],
            brow_inner_up: arr[16],
            brow_outer_up_left: arr[17], brow_outer_up_right: arr[18],
            nose_sneer_left: arr[19], nose_sneer_right: arr[20],
            cheek_puff: arr[21],
            cheek_squint_left: arr[22], cheek_squint_right: arr[23],
            mouth_close: arr[24], mouth_funnel: arr[25], mouth_pucker: arr[26],
            mouth_left: arr[27], mouth_right: arr[28],
            mouth_smile_left: arr[29], mouth_smile_right: arr[30],
            mouth_frown_left: arr[31], mouth_frown_right: arr[32],
            mouth_dimple_left: arr[33], mouth_dimple_right: arr[34],
            mouth_stretch_left: arr[35], mouth_stretch_right: arr[36],
            mouth_roll_lower: arr[37], mouth_roll_upper: arr[38],
            mouth_shrug_lower: arr[39], mouth_shrug_upper: arr[40],
            mouth_press_left: arr[41], mouth_press_right: arr[42],
            mouth_lower_down_left: arr[43], mouth_lower_down_right: arr[44],
            mouth_upper_up_left: arr[45], mouth_upper_up_right: arr[46],
            jaw_forward: arr[47], jaw_left: arr[48], jaw_right: arr[49], jaw_open: arr[50],
            tongue_out: arr[51],
        }
    }
...
impl IKSolver {
    ...
    pub fn solve(
        ...
        match chain.chain_type {
            IKChainType::TwoBone => self.solve_two_bone(chain, bone_positions, bone_rotations, target),
    ...
    fn solve_two_bone(
        ...
        let cos_angle = ((len_a * len_a + len_b * len_b - target_dist * target_dist) / (2.0 * len_a * len_b)).clamp(-1.0, 1.0);
        let mid_angle = std::f32::consts::PI - cos_angle.acos();
        
        // Angle at root
        let cos_root = ((len_a * len_a + target_dist * target_dist - len_b * len_b) / (2.0 * len_a * target_dist)).clamp(-1.0, 1.0);
        let _root_angle = cos_root.acos();
        
        // Calculate new positions
        let forward = target_vec.normalize();
        let pole = chain.bone_lengths.get(0)
    ...
    pub fn retarget(
        ...
        for mapping in &self.mappings {
            if let Some(source_rot) = source_rotations.get(&mapping.source) {
                // Get rest poses
                let source_rest = self.source_rest_pose
                    .get(&mapping.source)
                    .copied()
                    .unwrap_or(na::UnitQuaternion::identity());
                let target_rest = self.target_rest_pose
                    .get(&mapping.target)
                    .copied()
                    .unwrap_or(na::UnitQuaternion::identity());
...
impl AnimationDriver {
    ...
    pub fn update_blendshapes(&mut self, raw_blendshapes: &ARKitBlendshapes) -> ARKitBlendshapes {
        // Apply EMA smoothing
        let smoothed = self.prev_blendshapes.blend(raw_blendshapes, self.blendshape_smoothing);
    ...
    pub fn apply_ik(
        ...
        self.ik_solver.solve(chain_name, positions, rotations, target)
    }
}
...
mod tests {
    ...
    fn test_blendshape_blend() {
        let a = ARKitBlendshapes { eye_blink_left: 0.0, ..Default::default() };
        let b = ARKitBlendshapes { eye_blink_left: 1.0, ..Default::default() };
    ...
    fn test_ik_chain_creation() {
        ...
        solver.add_chain("left_arm", IKChain {
