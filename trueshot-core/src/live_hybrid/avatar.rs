...
impl Avatar {
    ...
    pub fn set_blendshape(&mut self, preset: BlendshapePreset, weight: f32) {
        self.blendshape_weights.insert(preset, weight.clamp(0.0, 1.0));
    }
...
impl BoundAvatar {
    ...
    pub fn apply_pose(&mut self, pose: &DetectedPose, smoothing: f32) {
        ...
        for (bone_name, &position) in &pose.joint_positions {
            let smoothed = self.smoothed_joints
                .entry(*bone_name)
                .or_insert(position);
            
            *smoothed = na::Point3::from(
                smoothed.coords.lerp(&position.coords, 1.0 - smoothing)
            );
        }
        
        // Solve IK for limbs
        self.solve_arm_ik(BoneName::LeftShoulder, BoneName::LeftUpperArm, BoneName::LeftHand);
        self.solve_arm_ik(BoneName::RightShoulder, BoneName::RightUpperArm, BoneName::RightHand);
        self.solve_leg_ik(BoneName::LeftUpperLeg, BoneName::LeftLowerLeg, BoneName::LeftFoot);
        self.solve_leg_ik(BoneName::RightUpperLeg, BoneName::RightLowerLeg, BoneName::RightFoot);
        ...
        if let Some(&head_pos) = self.smoothed_joints.get(&BoneName::Head) {
            if let Some(&neck_pos) = self.smoothed_joints.get(&BoneName::Neck) {
                ...
                self.current_skeleton.set_bone_rotation(BoneName::Head, head_rotation);
            }
        }
        ...
        // Update confidence
        let avg_confidence: f32 = pose.confidences.values().sum::<f32>() 
            / pose.confidences.len().max(1) as f32;
    ...
    fn solve_arm_ik(&mut self, _shoulder: BoneName, upper: BoneName, hand: BoneName) {
        let Some(&target) = self.smoothed_joints.get(&hand) else { return };
        let Some(&upper_pos) = self.smoothed_joints.get(&upper) else { return };
    ...
    fn solve_leg_ik(&mut self, upper: BoneName, _lower: BoneName, foot: BoneName) {
        let Some(&target) = self.smoothed_joints.get(&foot) else { return };
        let Some(&upper_pos) = self.smoothed_joints.get(&upper) else { return };
    ...
    pub fn get_bone_world_transform(&self, name: BoneName) -> Option<na::Matrix4<f32>> {
        self.current_skeleton.bone_name_to_index.get(&name)
    ...
    pub fn get_skinning_matrices(&self) -> Vec<[f32; 16]> {
        self.current_skeleton.skinning_matrices.iter()
            .map(|m| {
...
impl PoseEstimator {
    ...
    pub fn estimate_pose(
        ...
        joints.insert(BoneName::Neck, na::Point3::new(center.x, head_y - height * 0.08, center.z));
        confidences.insert(BoneName::Neck, 0.7);
        
        // Chest
        let chest_y = hips_y + height * 0.2;
        joints.insert(BoneName::Chest, na::Point3::new(center.x, chest_y, center.z));
        confidences.insert(BoneName::Chest, 0.6);
        
        // Shoulders
        let shoulder_width = height * 0.25;
        joints.insert(BoneName::LeftShoulder, na::Point3::new(center.x + shoulder_width, chest_y + height * 0.1, center.z));
        joints.insert(BoneName::RightShoulder, na::Point3::new(center.x - shoulder_width, chest_y + height * 0.1, center.z));
        ...
        joints.insert(BoneName::LeftHand, na::Point3::new(center.x + shoulder_width * 1.5, chest_y - height * 0.1, center.z));
        joints.insert(BoneName::RightHand, na::Point3::new(center.x - shoulder_width * 1.5, chest_y - height * 0.1, center.z));
        ...
        joints.insert(BoneName::LeftFoot, na::Point3::new(center.x + hip_width, foot_y, center.z));
        joints.insert(BoneName::RightFoot, na::Point3::new(center.x - hip_width, foot_y, center.z));
