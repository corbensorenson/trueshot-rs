    ...
    pub fn put(&self, path: PathBuf, data: Array3<f64>) -> Result<()> {
        ...
        // Check if we need to evict entries
        {
            ...
            if *current_size + size_bytes > self.max_size_bytes {
                tracing::info!("Bayer cache full ({} MB), clearing cache", *current_size / 1024 / 1024);
                cache.clear();
                *current_size = 0;
            }
            
            // Add new entry
            cache.insert(path.clone(), CachedBayer {
                data: data.clone(),
                size_bytes,
            });
            *current_size += size_bytes;
            
            tracing::debug!("Cached Bayer data for {:?} ({} MB, total cache: {} MB)",
    ...
    F: FnOnce() -> Result<Array3<f64>>,
{
    ...
    if let Some(cached) = cache.get(path) {
        tracing::debug!("Bayer cache HIT for {:?}", path.file_name().unwrap_or_default());
        return Ok(cached);
    }
    
    // Cache miss - load and cache
    tracing::debug!("Bayer cache MISS for {:?}", path.file_name().unwrap_or_default());
    let data = loader()?;
