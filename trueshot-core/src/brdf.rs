...
pub fn estimate_roughness(
    ...
    if num_planes < 3 {
        roughness.fill(0.5);
        specular.fill(0.5);
        return Ok(BrdfMaps { roughness, specular });
    }
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            let (peak_idx, peak_val) = curve.iter()
                .enumerate()
            ...
            let peak_sharpness = if fwhm > 0.0 { peak_val / fwhm } else { *peak_val };
    ...
    Ok(BrdfMaps { roughness, specular })
}
