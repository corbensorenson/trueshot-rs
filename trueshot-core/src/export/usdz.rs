...
pub fn export_usdz_with_options(mesh: &Mesh, path: &Path, options: &UsdExportOptions) -> Result<()> {
    let asset_name = path
        .file_name()
        .and_then(|s| s.to_str())
    ...
    let file = File::create(path).with_context(|| format!("Failed to create USDZ: {}", path.display()))?;
