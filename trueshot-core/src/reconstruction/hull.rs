...
use crate::mesh::{VoxelGrid, MarchingCubes, MarchingCubesConfig, ExtractedMesh};
...
pub fn carve_visual_hull(
    ...
    for voxel in voxels {
         ...
         for (i, mvp) in mvps.iter().enumerate() {
             let proj = mvp * p;
             // Perspective Divide
             if proj.w <= 0.0 { continue; } // Behind camera
