...
pub fn detect_object_bbox_with_key(
    ...
    detect_object_bbox_with_parser(&mut parser)
}

/// Detect an object using an already parsed NEF.
///
/// Callers that will subsequently load the RAW ROI should use this entry point
/// to avoid opening and parsing the same file twice.
pub fn detect_object_bbox_with_parser(parser: &mut Z9NefParser) -> Result<Rect> {
    ...
    let preview_jpeg = parser
        .extract_preview_jpeg()
        .context("Failed to extract preview JPEG")?;

    tracing::debug!(
        "Preview extraction: {:.1}ms",
        preview_start.elapsed().as_secs_f64() * 1000.0
    );
    detect_object_bbox_from_preview(&preview_jpeg, full_width, full_height)
}

fn detect_object_bbox_from_preview(
    preview_jpeg: &[u8],
    full_width: usize,
    full_height: usize,
) -> Result<Rect> {
    // Use JPEG IDCT scaling so the full-resolution embedded preview
    // never expands into a 100+ MB RGB buffer just to compute a bounding box.
    let (gray_data, preview_width, preview_height) =
        decode_preview_luma(preview_jpeg, 1024).context("Failed to decode scaled preview JPEG")?;
    if preview_width == 0 || preview_height == 0 {
        anyhow::bail!(
            "Preview image has invalid dimensions: {}x{}",
            preview_width,
            preview_height
        );
    }
    ...
    // Scale to full resolution using actual sensor metadata.
    ...
    tracing::info!(
        "Scaled bbox (before padding): {}x{} at ({}, {})",
        width,
        height,
        x,
        y
    ...
    tracing::info!(
        "ASYMMETRIC Padding: top={:.0}, left={:.0}, right={:.0}, bottom={:.0}",
        final_padding_top,
        final_padding_left,
        final_padding_right,
        final_padding_bottom
    );

    let x_with_padding = (x - final_padding_left).max(0.0);
    let y_with_padding = (y - final_padding_top).max(0.0);
    let width_with_padding =
        (width + final_padding_left + final_padding_right).min(full_width as f64 - x_with_padding);
    let height_with_padding = (height + final_padding_top + final_padding_bottom)
        .min(full_height as f64 - y_with_padding);
    ...
    tracing::info!(
        "Bayer-aligned ROI: {}x{} at ({}, {}) - {:.1}% of full image",
        final_rect.width,
        final_rect.height,
        final_rect.x,
        final_rect.y,
...
fn decode_preview_luma(jpeg: &[u8], target_edge: u16) -> Result<(Vec<u8>, usize, usize)> {
    let mut decoder = jpeg_decoder::Decoder::new(Cursor::new(jpeg));
    decoder
        .scale(target_edge, target_edge)
        .context("Unable to configure scaled JPEG decode")?;
    let pixels = decoder.decode().context("Scaled JPEG decode failed")?;
    let info = decoder
        .info()
        .context("Scaled JPEG decoder returned no image metadata")?;
    let width = info.width as usize;
    let height = info.height as usize;
    let pixel_count = width
        .checked_mul(height)
        .context("Scaled JPEG dimensions overflow")?;

    let luma = match info.pixel_format {
        jpeg_decoder::PixelFormat::L8 => {
            if pixels.len() != pixel_count {
                anyhow::bail!("Unexpected grayscale JPEG buffer length");
            }
            pixels
        }
        jpeg_decoder::PixelFormat::RGB24 => {
            if pixels.len() != pixel_count * 3 {
                anyhow::bail!("Unexpected RGB JPEG buffer length");
            }
            pixels
                .chunks_exact(3)
                .map(|rgb| {
                    ((77u32 * rgb[0] as u32 + 150u32 * rgb[1] as u32 + 29u32 * rgb[2] as u32) >> 8)
                        as u8
                })
                .collect()
        }
        other => anyhow::bail!("Unsupported preview JPEG pixel format: {:?}", other),
    };
...
fn calculate_otsu_threshold(gray: &[u8]) -> u8 {
    ...
    for (t, &count) in histogram.iter().enumerate() {
        ...
        if variance > max_variance {
            ...
            // Empty histogram bins create an equal-quality plateau. Its
            // midpoint is more stable than biasing the mask to the dark edge.
            threshold_end = t;
        }
    }

    ((threshold_start + threshold_end) / 2) as u8
}
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            if !visited[idx] && binary[idx] == 255 {
                ...
                if component.area > 0 {
                    // Keep all components, filter in scoring
...
fn find_largest_component_bbox(
    ...
    tracing::info!(
        "Evaluating {} components for bone detection",
        components.len()
    );
    ...
    for (i, component) in components.iter().enumerate() {
        ...
        tracing::debug!(
            "Component {}: area={} pixels ({:.1}%), bbox=({},{}) {}x{}",
            i,
            component.area,
            area_ratio * 100.0,
            component.min_x,
        ...
        if !(0.001..=0.6).contains(&area_ratio) {
            tracing::debug!(
                "Component {} rejected: area ratio {:.1}% outside range [0.1%, 60%]",
                i,
                area_ratio * 100.0
            );
            continue;
        ...
        let center_distance =
            ((center_x - image_center_x).powi(2) + (center_y - image_center_y).powi(2)).sqrt();
        ...
        let shape_score = if aspect_ratio > 0.2 && aspect_ratio < 5.0 {
            1.0
        } else {
            0.7
        };
        ...
        let size_score = if area_ratio > 0.01 && area_ratio < 0.3 {
            1.0
        } else {
            0.8
        };
        ...
        tracing::debug!(
            "Component {} score: {:.3} (size={:.2}, center={:.2}, shape={:.2})",
            i,
            total_score,
            size_score,
            center_score,
    ...
    if let Some(component) = best_component {
        tracing::info!(
            "Selected component: area={}, coverage={:.3}, score={:.3}",
            component.area,
            component.area as f32 / total_pixels,
            best_score
        );
