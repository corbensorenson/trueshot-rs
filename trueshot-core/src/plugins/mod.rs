...
impl PluginEngine {
    pub fn new() -> Result<Self> {
        ...
        // Host Functions
        linker.func_wrap("host", "log", |mut caller: Caller<'_, ContextState>, ptr: i32, len: i32| {
    ...
    pub fn process_image(&self, wasm_path: &Path, image_data: &[u8]) -> Result<Vec<u8>> {
        ...
        let memory = instance.get_memory(&mut store, "memory").context("Plugin must export memory")?;
        ...
        let process_fn = instance.get_typed_func::<(i32, i32), i32>(&mut store, "process_image")
        ...
        let get_len_fn = instance.get_typed_func::<i32, i32>(&mut store, "get_len")
