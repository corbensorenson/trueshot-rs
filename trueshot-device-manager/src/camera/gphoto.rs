...
use gphoto2::{Context as GpContext, Camera as GpCamera, widget::Widget};
use std::path::PathBuf;
use std::sync::{Arc, Mutex};
use super::{Camera, CameraConfig, CameraCapabilities};
...
impl GPhotoCamera {
    pub fn new(camera: GpCamera, context: GpContext) -> Result<Self> {
        ...
        let cam_arc = Arc::new(Mutex::new(camera));
        {
            ...
            if let Ok(config) = block_on(cam.config()) {
                 ...
                 let get_choices = |label: &str| -> Vec<String> {
                     match config.get_child_by_label(label) {
                         Ok(widget) => {
                             ...
                             match widget {
                                 Widget::Radio(radio) => {
                                     let choices: Vec<String> = radio.choices_iter().map(|x| x.to_string()).collect();
                                     tracing::info!("Found Radio widget '{}' with {} choices", label, choices.len());
                                     return choices;
                                 },
                                 other => {
                                     tracing::info!("Found widget '{}' but it's not Radio, it's {:?}", label, std::mem::discriminant(&other));
                                 }
                             }
                         },
                         Err(e) => {
                 ...
                 tracing::info!("Capabilities populated: ISO={}, Shutter={}, Aperture={}, WB={}", 
    ...
    pub fn detect_all() -> Result<Vec<Self>> {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            for i in 0..3 {
                let output = std::process::Command::new("killall").arg("-9").arg("PTPCamera").output();
                match output {
                    Ok(out) => {
                        if out.status.success() {
                            tracing::error!("Successfully killed PTPCamera (Attempt {})", i + 1);
                        } else {
                            tracing::error!("PTPCamera kill attempted but process not found or failed: {:?}", out);
                        }
                    },
                    Err(e) => {
                 ...
                 let _ = std::process::Command::new("killall").arg("-9").arg("gphoto2").output();
...
impl Camera for GPhotoCamera {
    ...
    fn capture(&self, _config: &CameraConfig) -> Result<PathBuf> {
        ...
        let _path_info = block_on(cam.capture_image())
    ...
    fn capture_preview(&self) -> Result<Vec<u8>> {
        let cam = self.camera.lock().unwrap();
        // Capture preview
        let file = block_on(cam.capture_preview())
    ...
    fn set_config(&self, config: &CameraConfig) -> Result<()> {
        let cam = self.camera.lock().unwrap();
        let gp_config = block_on(cam.config())
        ...
        if let Some(wb) = &config.wb {
            let wb_labels = ["White Balance", "White balance", "whitebalance", "WhiteBalance", "WB"];
    ...
    fn drive_focus(&self, step: i32) -> Result<()> {
        ...
        let config = block_on(cam.config())
            .map_err(|e| anyhow!("Failed to get config: {}", e))?;
        ...
        if let Ok(widget) = config.get_child_by_label("Drive Nikon DSLR Manual focus") {
            if let Widget::Range(range) = widget {
                ...
                let _ = range.set_value(focus_val);
        ...
        if let Ok(widget) = config.get_child_by_name(widget_name) {
             let value_str = if step > 0 {
                 if step > 5 { "Far 3" } else if step > 2 { "Far 2" } else { "Far 1" }
             } else {
                 if step < -5 { "Near 3" } else if step < -2 { "Near 2" } else { "Near 1" }
             };
             
             match widget {
                 Widget::Range(range) => {
                     let val_f32 = step as f32;
                     let _ = range.set_value(val_f32);
                 },
                 Widget::Radio(radio) => {
