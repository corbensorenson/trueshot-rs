...
pub use registry::{
    CalibrationData, CameraCapabilities, CameraRegistry, CameraRole, CameraSettings,
    ColorCalibrationData,
};

pub use insta360::Gimbal;
pub use insta360::Insta360Link;
pub use kinect::{DepthCamera, KinectCamera, KinectLedColor, KinectStreamType};
...
pub trait Camera: Send + Sync {
    ...
    fn as_depth_camera(&self) -> Option<&dyn DepthCamera> {
        None
    }
...
impl LazyNokhwaCamera {
    ...
    fn ensure_init(&self) -> Result<()> {
        ...
        let idx_u32 = if let nokhwa::utils::CameraIndex::Index(i) = idx {
            i
        } else {
            0
        };
        let result = std::panic::catch_unwind(|| {
            if is_insta {
                Insta360Link::new(idx_u32)
                    .ok()
                    .map(|c| Box::new(c) as Box<dyn Camera>)
            } else {
                GenericWebcam::new(idx_u32, &name)
                    .ok()
                    .map(|c| Box::new(c) as Box<dyn Camera>)
            }
        });
...
impl CameraManager {
    ...
    pub async fn reconcile_cameras(&mut self, _include_mock: bool) -> Result<ReconciliationReport> {
        ...
        enum Discovered {
            #[cfg(feature = "gphoto2")]
            GPhoto(Box<gphoto::GPhotoCamera>),
            Nokhwa {
                id: String,
                index: nokhwa::utils::CameraIndex,
                name: String,
        ...
        let discovered_list = tokio::task::spawn_blocking(move || {
            ...
            if gphoto_active {
                ...
                match gphoto::GPhotoCamera::detect_all() {
                    Ok(cams) => {
                        ...
                        for c in cams {
                            list.push(Discovered::GPhoto(Box::new(c)));
                        }
                    }
            ...
            let query_result =
                std::panic::catch_unwind(|| nokhwa::query(nokhwa::utils::ApiBackend::Auto));
            ...
            match query_result {
                Ok(Ok(cameras)) => {
                    for cam_info in cameras {
                        ...
                        tracing::info!(
                            "Nokhwa Found Candidate: '{}' (Index: {:?})",
                            name,
                            cam_info.index()
                        );

                        if name_lower.contains("facetime")
                            || name_lower.contains("continuity")
                            || name.eq("Capture")
                        {
                        ...
                        if let nokhwa::utils::CameraIndex::Index(idx) = cam_info.index() {
                            ...
                            list.push(Discovered::Nokhwa {
                                id,
                                index: cam_info.index().clone(),
                                name,
                                is_insta,
                            });
                        }
        ...
        let mut new_map: std::collections::HashMap<String, Discovered> =
        ...
        for cam in old_cameras {
            ...
            if let Err(e) = cam.battery_level() {
                tracing::warn!(
                    "Existing camera {} failed health check: {}. Removing.",
                    id,
                    e
                );
    ...
    pub fn trigger_all(&self, config: &CameraConfig) -> Vec<Result<PathBuf>> {
        self.cameras.iter().map(|cam| cam.capture(config)).collect()
    }
