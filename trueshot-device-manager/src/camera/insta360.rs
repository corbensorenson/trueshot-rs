...
impl Insta360Link {
    pub fn new(index: u32) -> Result<Self> {
        ...
        if camera.is_none() {
            tracing::error!("Failed to initialize Insta360 camera stream. Proceeding in Gimbal-Only mode.");
        }
...
impl TetherCamera for Insta360Link {
        ...
        if let Some(cam_mutex) = &self.camera {
             if let Some((w, h)) = config.resolution {
                 ...
                 if current.resolution() != Resolution::new(w, h) {
                     ...
                     cam.stop_stream()?;
                     let new_fmt = RequestedFormat::new::<RgbFormat>(
                        RequestedFormatType::Closest(CameraFormat::new(Resolution::new(w, h), FrameFormat::MJPEG, 30))
                     );
        ...
        if let Some(tool) = tool_path {
            ...
            if zoom > 0.0 {
                 let _ = std::process::Command::new(tool)
                    .arg("-N").arg("Insta360 Link")
                    .arg("-s").arg(&format!("zoom-abs={}", zoom_val))
                    .output();
            }
        }
        Ok(())
    ...
    fn as_gimbal(&self) -> Option<&dyn Gimbal> { Some(self) }
}
...
impl Gimbal for Insta360Link {
    fn set_pan_tilt(&self, pan_deg: f32, tilt_deg: f32) -> Result<()> {
        ...
        tracing::info!("Setting PT: P={} ({}), T={} ({})", pan_deg, pan_val, tilt_deg, tilt_val);
        ...
        let tool_command = possible_paths.iter().find(|p| p.exists())
        ...
        let output = std::process::Command::new(&tool_command)
            ...
            .arg("-s")
            .arg(&set_arg)
            .output().map_err(|e| anyhow!("Failed to spawn uvc-util: {}", e))?;
