...
impl Default for KinectLedColor {
    fn default() -> Self {
        KinectLedColor::Green
    }
}
...
impl Default for KinectStreamType {
    fn default() -> Self {
...
impl KinectCamera {
    ...
    pub fn set_stream_type(&self, stream: KinectStreamType) -> Result<()> {
        let mut current = self.current_stream.lock()
    ...
    pub fn get_stream_type(&self) -> Result<KinectStreamType> {
        let current = self.current_stream.lock()
            .map_err(|e| anyhow!("Lock error: {}", e))?;
        Ok(*current)
    }
        ...
        let meters: Vec<f32> = raw.iter()
            .map(|&d| {
                if d == 0 || d == 2047 {
                    ...
                    Self::DEPTH_MIN_METERS + normalized * (Self::DEPTH_MAX_METERS - Self::DEPTH_MIN_METERS)
                }
            })
            .collect();
...
impl TetherCamera for KinectCamera {
    ...
    fn capture(&self, _config: &CameraConfig) -> Result<PathBuf> {
        ...
        let filename = format!("kinect_{}_{:?}_{}.png", 
            self.device_index, 
            stream_type,
    ...
    fn capture_preview(&self) -> Result<Vec<u8>> {
        ...
        match stream_type {
            ...
            KinectStreamType::Infrared => {
                ...
                dynamic.write_to(&mut std::io::Cursor::new(&mut bytes), image::ImageFormat::Jpeg)?;
                Ok(bytes)
            }
        }
    }
