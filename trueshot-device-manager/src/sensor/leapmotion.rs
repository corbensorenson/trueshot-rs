...
use super::{Sensor, SensorType, SensorCapabilities, SensorData};
...
impl Default for LeapMotionMode {
    fn default() -> Self {
...
impl LeapMotionController {
    ...
    fn initialize(&self) -> Result<()> {
        ...
        thread::spawn(move || {
            tracing::info!("Leap Motion tracking thread started for device {}", device_index);
    ...
    pub fn compute_disparity(&self) -> Result<Vec<u8>> {
        ...
        for y in half_block..(height - half_block) {
            for x in (half_block + max_disparity)..(width - half_block) {
                ...
                disparity[y * width + x] = ((max_disparity - best_disparity) * 255 / max_disparity) as u8;
            }
        }
    ...
    pub fn generate_point_cloud(&self) -> Result<Vec<(f32, f32, f32)>> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if d > 10.0 {  // Minimum disparity threshold
...
mod tests {
    ...
    fn test_gesture_interpretation() {
        let mut hand = TrackedHand::default();
        
        hand.grab_strength = 0.95;
        assert_eq!(LeapMotionController::interpret_gesture(&hand), HandGesture::Fist);
        
        hand.grab_strength = 0.1;
        hand.pinch_strength = 0.9;
        assert_eq!(LeapMotionController::interpret_gesture(&hand), HandGesture::Pinch);
        
        hand.grab_strength = 0.1;
        hand.pinch_strength = 0.1;
        assert_eq!(LeapMotionController::interpret_gesture(&hand), HandGesture::OpenHand);
    }
}
