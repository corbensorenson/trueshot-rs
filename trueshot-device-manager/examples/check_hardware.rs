        ...
        Ok(report) => {
            ...
            for cam in &manager.cameras {
                ...
                if let Some(profile) = manager.registry.get_profile(&id) {
                     println!("    ID: {:<20} | Name: {:<20} | Role: {:?}", id, profile.name, profile.role);
                } else {
                 ...
                 if let Some(central) = adapters.pop() {
                     ...
                     if peripherals.is_empty() {
                         ...
                         for p in peripherals {
                             ...
                             let local_name = props.clone().and_then(|pr| pr.local_name).unwrap_or_else(|| "Unknown".to_string());
                             if local_name.to_lowercase().contains("foldio") {
                                 println!("      ✅ Found Turntable: {}", local_name);
                                 println!("      ✅ Found Turntable: {}", local_name);
                             } else {
                                 println!("      Found Other Device: {} (Address: {})", local_name, p.address()); 
