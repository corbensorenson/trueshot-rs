...
| Capture | Uncertainty-driven HDR/focus planner | In Progress | Calibrated deterministic core parses bounded camera option grids, ranks exact-ISO shutter/focus candidates by coupled posterior entropy reduction per millisecond, stops HDR/focus independently on explicit quality targets, records every utility/rejection/selection/termination in the streaming manifest, and cuts modeled time 82.7% versus a fixed 21-shot grid at equal synthetic variance targets; live probe extraction, calibrated lens-drive feedback, server camera adapter, and real capture-time gates remain |
