...
| Processing | Calibrated probabilistic RAW fusion | In Progress | Exact-ISO/CFA profiles, censored Poisson-Gaussian likelihood, posterior uncertainty, source attribution, clipping/rejection/fallback flags, and fail-closed gates are implemented; calibration capture/persistence, empirical coverage, selective local alignment, and product overlays remain |
