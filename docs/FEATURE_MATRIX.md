...
| Processing | Calibrated probabilistic RAW fusion | In Progress | Exact-ISO/CFA schema-versioned profiles, atomic persistence, SHA-256 artifact identity, production CLI loading, censored likelihood, posterior uncertainty, exact source/flag maps, bounded visible provenance overlay, traceable fallback, and fail-closed gates are implemented; calibration capture/fit and empirical uncertainty coverage remain |
