...
| Processing | Physical PSF-aware focus fusion | In Progress | Native fusion validates lens/sensor geometry, fits bounded-memory sub-plane peaks on nonuniform diopters, applies thin-lens PSF confidence, projects the continuous sensor surface onto the conservative aperture-valid set, constructs a tri-state physical mixed-pixel trimap with a variable-radius linear transform, anchors ambiguous support to one measured focus plane, tags correction/fallback provenance, and exports unquantized meter depth. Held-out real-lens calibration, breathing, and real hair/fur/transparency halo qualification remain |
