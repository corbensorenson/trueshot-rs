...
| Processing | Physical PSF-aware focus fusion | In Progress | Native fusion validates lens/sensor geometry and fits bounded-memory sub-plane peaks on nonuniform diopters. A digest-bound `trueshot.lens-psf.v1` profile models focus breathing, entrance pupil, and residual field PSF from source-hashed fit/holdout measurements; camera/sensor/lens/aperture/focal/focus mismatches fail closed. Calibrated image distance drives metric depth, field PSF drives confidence and crossing support, and the conservative effective pupil drives aperture-valid projection. The tri-state mixed-pixel transform anchors ambiguous support to one measured plane and exports profile/fallback provenance plus unquantized meter depth. No profile is explicitly reported as ideal-only. Retained real-lens profiles and real hair/fur/transparency halo/MTF qualification remain |
