...
| Capture | Uncertainty-driven HDR/focus planner | In Progress | Calibrated deterministic core ranks exact-ISO shutter/focus candidates by coupled HDR/focus posterior entropy reduction per millisecond with motion, time, thermal, SNR, and lens-travel constraints plus independent stopping; camera-loop integration and real capture-time gates remain |
| Processing | Native HDR + focus fusion | In Progress | Tiled `f32` fusion consumes the `u16` arena directly with lazy exposure/WB calibration, subpixel alignment, median/MAD deghosting, scale-decoupled UHD focus evidence, confidence, and depth-consistent aperture-valid refusion; CLI strength/throughput controls exist, while direct Helicon/Lightroom qualification remains |
| Processing | Deterministic focus/HDR quality gates | In Progress | Scale-decoupled synthetic baseline is 44.161 dB PSNR and 100% depth accuracy with a 44 dB floor plus radiance/deghost/CFA/visibility invariants; redistributable real corpus and direct competitor gates remain |
| Processing | Calibrated probabilistic RAW fusion | In Progress | Exact-ISO/CFA schema-versioned profiles, atomic persistence, SHA-256 artifact identity, production CLI loading, censored likelihood, posterior uncertainty, source attribution, and fail-closed gates are implemented; calibration capture/fit, empirical coverage, selective local alignment, and overlays remain |
