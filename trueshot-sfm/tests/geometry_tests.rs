...
use trueshot_sfm::geometry::{RansacConfig, ransac_essential, ransac_homography};
use nalgebra as na;
...
fn test_ransac_homography_finds_correct_model() {
    ...
    assert!(result.is_some(), "RANSAC should find simple translation homography");
}
...
fn test_ransac_homography_with_outliers() {
    ...
    for _ in 0..5 {
        pts1.push(na::Point2::new(rng.gen_range(100.0..400.0), rng.gen_range(100.0..400.0)));
        pts2.push(na::Point2::new(rng.gen_range(100.0..400.0), rng.gen_range(100.0..400.0)));
    }
    ...
    if let Some(r) = result {
        assert!(r.inlier_ratio > 0.3, "Should find significant inliers: {}", r.inlier_ratio);
    }
...
fn test_ransac_returns_none_on_insufficient_points() {
    ...
    let result = ransac_homography(&pts1, &pts2, &config);
    assert!(result.is_none(), "Should return None with insufficient points");
}
...
fn test_ransac_essential_with_camera_motion() {
    ...
    assert!(result.is_some(), "Should find essential matrix for valid motion");
    
    let result = result.unwrap();
    assert!(result.inlier_ratio > 0.7, "Most points should be inliers: {}", result.inlier_ratio);
}
...
fn test_essential_matrix_rank() {
    ...
    if let Some(result) = ransac_essential(&pts1, &pts2, &K, &K, &config) {
        ...
        assert!(singular_values[2].abs() < 1e-6, 
            "Third singular value should be ~0: {}", singular_values[2]);
        
        // First two should be approximately equal
        let ratio = singular_values[0] / singular_values[1];
        assert!((ratio - 1.0).abs() < 0.1, 
            "First two singular values should be ~equal: {}", ratio);
    }
}
