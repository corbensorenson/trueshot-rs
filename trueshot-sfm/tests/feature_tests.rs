...
use trueshot_sfm::features::{detect_orb, detect_sift, Keypoint, Descriptor};
...
fn test_orb_detects_features_on_textured_image() {
    ...
    assert!(!keypoints.is_empty(), "ORB should detect features on checkerboard");
    assert!(keypoints.len() >= 10, "Should detect at least 10 features, got {}", keypoints.len());
    
    // Keypoints and descriptors should match
    assert_eq!(keypoints.len(), descriptors.len(), "Keypoint/descriptor count mismatch");
}
...
fn test_orb_respects_max_features() {
    ...
    assert!(keypoints.len() <= max_features, 
        "Should not exceed max features: got {} > {}", keypoints.len(), max_features);
}
...
fn test_orb_keypoints_have_valid_coordinates() {
    ...
    for kp in &keypoints {
        assert!(kp.x >= 0.0 && kp.x < 640.0, "Invalid x coordinate: {}", kp.x);
        assert!(kp.y >= 0.0 && kp.y < 480.0, "Invalid y coordinate: {}", kp.y);
        assert!(kp.scale > 0.0, "Scale should be positive: {}", kp.scale);
        assert!(kp.response > 0.0, "Response should be positive: {}", kp.response);
    }
}
...
fn test_orb_on_uniform_image_gives_few_features() {
    ...
    assert!(keypoints.len() < 50, 
        "Uniform image should have few features, got {}", keypoints.len());
}
...
fn test_descriptor_hamming_distance() {
    let desc1 = Descriptor { data: vec![0b11110000; 32] };
    let desc2 = Descriptor { data: vec![0b11110000; 32] };
    let desc3 = Descriptor { data: vec![0b00001111; 32] };
...
fn test_descriptor_l2_distance() {
    let desc1 = Descriptor { data: vec![100; 128] };
    let desc2 = Descriptor { data: vec![100; 128] };
    let desc3 = Descriptor { data: vec![200; 128] };
...
fn test_feature_detection_is_deterministic() {
    ...
    assert_eq!(kp1.len(), kp2.len(), "Feature count should be deterministic");
