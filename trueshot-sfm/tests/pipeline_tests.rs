...
fn test_pipeline_add_images() {
    ...
    assert!(
        result.is_ok(),
        "Should successfully add images: {:?}",
        result.err()
    );
}
...
fn test_reconstruction_export_ply() {
    ...
    use nalgebra as na;
    use trueshot_sfm::{CameraIntrinsics, CameraPose, Point3D};
    ...
    assert!(
        content.contains("element vertex 2"),
...
fn test_camera_pose_identity() {
    ...
    assert!(
        angle.abs() < 1e-10,
        "Identity rotation should have zero angle"
    );

    // Zero translation
    assert!(
        pose.translation.norm() < 1e-10,
        "Identity translation should be zero"
    );
}
