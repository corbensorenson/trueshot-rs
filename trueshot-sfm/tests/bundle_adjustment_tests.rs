...
use trueshot_sfm::optimization::{
    BundleAdjustmentConfig, Observation, bundle_adjust_lm,
};
use trueshot_sfm::{Point3D, CameraPose, CameraIntrinsics, DistortionModel};
use nalgebra as na;

/// Create a simple test scene with known geometry
fn create_test_scene() -> (Vec<Point3D>, Vec<CameraPose>, Vec<CameraIntrinsics>, Vec<Observation>) {
...
fn test_bundle_adjustment_converges_on_perfect_data() {
    ...
    assert!(result.final_cost < 1.0, "Final cost should be small: {}", result.final_cost);
...
fn test_bundle_adjustment_improves_noisy_data() {
    ...
    assert!(result.final_cost <= result.initial_cost, 
        "Cost should not increase: {} > {}", result.final_cost, result.initial_cost);
}
...
fn test_bundle_adjustment_with_perturbed_poses() {
    ...
    assert!(result.final_cost <= result.initial_cost * 1.1, "Cost should not increase significantly");
}
...
fn test_bundle_adjustment_empty_input() {
    ...
    assert!(result.converged, "Should converge immediately on empty input");
    assert_eq!(result.iterations, 0);
}

#[test]
fn test_huber_loss_reduces_outlier_influence() {
    let (mut points, mut poses, intrinsics, mut observations) = create_test_scene();
    ...
    assert!(result_huber.rmse < 100.0, "RMSE with Huber should be reasonable: {}", result_huber.rmse);
}
