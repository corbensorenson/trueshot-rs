...
impl Mesh {
    ...
    pub fn export_ply(&self, path: impl AsRef<Path>) -> anyhow::Result<()> {
        ...
        for (i, v) in self.vertices.iter().enumerate() {
            let n = self.vertex_normals.get(i).copied().unwrap_or(na::Vector3::z());
            let c = self.vertex_colors.get(i).copied().unwrap_or([128, 128, 128]);
            writeln!(file, "{} {} {} {} {} {} {} {} {}",
...
pub fn marching_cubes_reconstruction(
    ...
    tracing::info!("Running Marching Cubes reconstruction with {} points, resolution {}", 
    ...
    tracing::info!("Mesh reconstruction complete: {} vertices, {} triangles",
        mesh.vertices.len(), mesh.triangles.len());
    
    Ok(mesh)
}

fn get_cube_corner(x: usize, y: usize, z: usize, corner: usize, min_pt: &na::Point3<f64>, voxel_size: f64) -> na::Point3<f64> {
    let offsets = [
...
fn get_marching_cubes_triangles(cube_index: usize) -> [[i32; 3]; 5] {
    ...
    match cube_index {
        ...
        1 => { result[0] = [0, 8, 3]; },
        2 => { result[0] = [0, 1, 9]; },
        3 => { result[0] = [1, 8, 3]; result[1] = [9, 8, 1]; },
        4 => { result[0] = [1, 2, 10]; },
        8 => { result[0] = [3, 11, 2]; },
        15 => { result[0] = [9, 10, 8]; result[1] = [10, 11, 8]; },
        _ => {
...
pub fn simplify_mesh(mesh: &mut Mesh, target_triangles: usize) -> anyhow::Result<()> {
    ...
    tracing::info!("Simplifying mesh from {} to {} triangles", mesh.triangles.len(), target_triangles);
