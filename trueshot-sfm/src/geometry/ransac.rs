...
pub fn ransac_essential(
    ...
    while iter < max_iterations {
        ...
        if inlier_ratio > config.min_inlier_ratio
            && best_result
                .as_ref()
                .map_or(true, |r| inliers.len() > r.inliers.len())
        {
            ...
            max_iterations =
                adaptive_iterations(inlier_ratio, 8, config.confidence).min(config.max_iterations);
        }
        iter += 1;
    }
    ...
    if let Some(mut result) = best_result {
        let inlier_pts1: Vec<na::Point2<f64>> =
            result.inliers.iter().map(|&i| norm_pts1[i]).collect();
        let inlier_pts2: Vec<na::Point2<f64>> =
            result.inliers.iter().map(|&i| norm_pts2[i]).collect();
...
fn normalize_points(pts: &[na::Point2<f64>]) -> (Vec<na::Point2<f64>>, na::Matrix3<f64>) {
    ...
    let normalized: Vec<na::Point2<f64>> = pts
        .iter()
