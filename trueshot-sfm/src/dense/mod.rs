...
pub fn patchmatch_stereo(input: &MvsInput, config: &PatchMatchConfig) -> DepthMap {
    ...
    tracing::info!(
        "Running PatchMatch stereo: {}x{}, {} source images",
        width,
        height,
        input.src_images.len()
    );
...
fn refine_random(depth_map: &mut DepthMap, input: &MvsInput, config: &PatchMatchConfig) {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let current_cost =
                compute_matching_cost(input, x, y, current_depth, current_normal, config);
            ...
            for _ in 0..config.num_samples {
                ...
                let new_depth =
                    (current_depth + delta_depth).clamp(config.depth_min, config.depth_max);
...
fn compute_matching_cost(
    ...
    let point_world =
        na::Point3::from(input.ref_pose.rotation * point_cam.coords + input.ref_pose.translation);
            ...
            for dy in -kernel_radius..=kernel_radius {
                for dx in -kernel_radius..=kernel_radius {
                    ...
                    if nx >= 0 && ny >= 0 && nx < width as i32 && ny < height as i32 {
                        ...
                        let spatial_weight = (-spatial_dist_sq
                            / (2.0 * config.bilateral_sigma_spatial.powi(2)))
                        .exp();
                        let depth_weight = (-depth_diff_sq
                            / (2.0 * config.bilateral_sigma_depth.powi(2) * center_depth.powi(2)))
                        .exp();
...
pub fn fuse_depth_maps(
    ...
    tracing::info!(
        "Fusing {} depth maps with min {} views",
        depth_maps.len(),
        min_views
    );
    ...
    for (ref_idx, depth_map) in depth_maps.iter().enumerate() {
        ...
        for y in 0..depth_map.height {
            for x in 0..depth_map.width {
                ...
                // Transform to world: R * p + t
                let point_world =
                    na::Point3::from(pose.rotation * point_cam.coords + pose.translation);
