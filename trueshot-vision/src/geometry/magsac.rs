...
impl Magsac {
    ...
    pub fn estimate<M: MagsacModel>(&self, data: &[M::Point]) -> Option<MagsacResult<M>> {
        ...
        for iter in 0..iterations {
            ...
            if score > best_score {
                ...
                if refined_score > best_score {
                    ...
                    if inlier_ratio > 0.1 {
                        iterations = self.adaptive_iterations(inlier_ratio, min_samples)
                            .min(iter + iterations / 2);
                    }
                }
            }
        }
        ...
        best_model.map(|model| {
            let inliers: Vec<usize> = best_weights.iter()
                .enumerate()
                .filter(|(_, &w)| w > 0.5)
                .map(|(i, _)| i)
                .collect();
    ...
    fn compute_magsac_score<M: MagsacModel>(&self, model: &M, data: &[M::Point]) -> (f64, Vec<f64>) {
        let n = data.len();
    ...
    fn irls_refine<M: MagsacModel>(&self, mut model: M, data: &[M::Point], initial_weights: &[f64]) -> M {
    ...
    fn adaptive_iterations(&self, inlier_ratio: f64, min_samples: usize) -> usize {
        ...
        if p >= 1.0 { return 1; }
        if p <= 0.0 { return self.config.max_iterations; }
...
mod tests {
    ...
    impl MagsacModel for LineModel {
        ...
        fn min_samples() -> usize { 2 }

        fn estimate(points: &[Self::Point]) -> Option<Self> {
            if points.len() < 2 { return None; }
            let (x1, y1) = points[0];
            let (x2, y2) = points[1];
            if (x2 - x1).abs() < 1e-10 { return None; }
    ...
    fn test_magsac_line() {
        ...
        assert!((result.model.intercept - 1.0).abs() < 0.5, "Intercept should be ~1");
