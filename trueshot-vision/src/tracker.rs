...
use opencv::{
    ...
    imgproc::{self, CHAIN_APPROX_SIMPLE, MORPH_CLOSE, MORPH_OPEN, RETR_EXTERNAL},
    objdetect,
    prelude::*,
    tracking,
...
impl ObjectTracker {
    ...
    pub fn track(&mut self, image: &ImageBuffer<Rgb<u8>, Vec<u8>>) -> Result<TrackedObject> {
        ...
        if self.bg_subtractor.is_none() {
            self.bg_subtractor = Some(video::create_background_subtractor_mog2(300, 25.0, false)?);
        ...
        if let Some(ref mut tracker) = self.csrt_tracker {
            ...
            if tracker.update(&mat, &mut bbox_rect)? {
                ...
                self.object_state.bounding_box =
...
impl ObjectTracker {
    ...
    pub fn track(
        &mut self,
        _image: &image::ImageBuffer<image::Rgb<u8>, Vec<u8>>,
